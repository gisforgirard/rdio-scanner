...
export class Controller extends EventEmitter {
  ...
  constructor(ctx) {
    ...
    if (ctx.debug == true) { // im not positive why this would ever be true lol
        this.debug = true;
    } else {
        this.debug = false;
    }
    ...
    //this.models.audio.belongsTo(this.models.call, { as: 'call_audio', foreignKey: 'call_id'});
    // associate audio with call
    this.models.call.hasOne(this.models.audio, {  foreignKey: 'call_id'});
    this.models.audio.belongsTo(this.models.call, {  foreignKey: 'call_id'});
    ...
    if (!this.ffmpeg) {
      this.log.write(
          Log.warn,
          'Controller: ffmpeg is missing, no audio conversion possible.');
    }
  ...
  broadcastConfig() {
    if (this.websocket) {
      this.websocket.wss.clients.forEach(async (socket) => {
        ...
        socket.send(
            JSON.stringify([wsCommand.config, this.getConfig(socket.scope)]));
      });
    }
  }
  ...
  convertCallAudio(call = {}) {
    return new Promise((resolve, reject) => {
      ...
      if (Buffer.isBuffer(call && call.audio)) {
        ...
        const system =
            this.config.systems.find((sys) => sys.id === call.system);

        if (system && Array.isArray(system.talkgroups)) {
          const talkgroup =
              system.talkgroups.find((tg) => tg.id === call.talkgroup);

          if (talkgroup) {
            metadata.push(
                ...['-metadata',
                    `album="${talkgroup.label}"`,
                    '-metadata',
                    `artist="${system.label}"`,
                    '-metadata',
                    `date="${call.dateTime}"`,
        ...
        const proc = spawn('ffmpeg', [
          ...
          '-af',
          'dynaudnorm=f=75:g=11:s=12:p=0.71:m=20.0',
          '-f',
          'ipod',
          '-',
        ]);
        ...
        proc.on('close', () => {
          call.audio = audio;
          call.audioName = call.audioName ?
              call.audioName.replace(/\.[^.]+$/, '.m4a') :
              'audio.m4a';
        ...
        proc.stdout.on(
            'data', (data) => (audio = Buffer.concat([audio, data])));
    ...
    const parse = (record) => {
      const parseSystem = (system, first = true) => {
        const parseTalkgroup = (talkgroup, first = true) => {
          if (Array.isArray(talkgroup)) {
            return talkgroup.map((tg) => parseTalkgroup(tg, false));
              ...
              talkgroup !== null && typeof talkgroup === 'object' &&
              typeof talkgroup.id === 'number') {
            return first ? [talkgroup.id] : talkgroup.id;
          } else if (typeof talkgroup === 'number') {
            return first ? [talkgroup] : talkgroup;
          } else if (talkgroup === '*') {
            const talkgroups =
                (this.config.systems.find((sys) => sys.id === system.id) ||
                 {}).talkgroups;
        ...
        if (Array.isArray(system)) {
          return system.map((sys) => parseSystem(sys, false))
              .filter((sys) => sys.length);
        } else if (
            system !== null && typeof system === 'object' &&
          ...
          if (this.config.systems.find((sys) => sys.id === system.id)) {
            ...
            if (talkgroups.length) {
              return first ? [[system.id, talkgroups]] :
                             [system.id, talkgroups];
            } else {
              return [];
            }
          } else {
          ...
          if (this.config.systems.find((sys) => sys.id === system)) {
            const parsed = parseSystem(
                this.config.systems.find((sys) => sys.id === system), false);
      ...
      const parseSystem = (system, first = true) => {
        const parseTalkgroup = (talkgroup, first = true) => {
          if (Array.isArray(talkgroup)) {
            ...
            const talkgroups =
                (systems.find((sys) => sys.id === system.id) || {}).talkgroups;
  ...
  async getCall(id, scope) {
   ...
   const where = id;
    ...
    var call = await this.models.call.findOne({where, 
      include: [{ model: this.models.audio }]});
      //call.audio = null;

      // total sequelize hack
      // this datavalues shit is fucking bullshit
      call.dataValues.audio = call.rdioScannerAudio.audio;
  ...
  async getCalls(options, scope) {
    ...
    // console.log(scope);
    if (scope !== null && typeof scope === 'object') {
      // this is literally retarded and there is no reason for this to exist
...
*/
    ...
    const date = options && typeof options.date === 'string' ?
        new Date(options.date) :
        null;

    const limit = options && typeof options.limit === 'number' ?
        Math.min(500, options.limit) :
        500;
    ...
    const order = [
      [
        'dateTime',
        options && typeof options.sort === 'number' && options.sort < 0 ?
            'DESC' :
            'ASC',
      ],
    ];
    ...
    const [dateStartQuery, dateStopQuery, count, results] = await Promise.all([
      await this.models.call.findOne({
        attributes: ['dateTime'],
        order: [['dateTime', 'ASC']],
        where: where1,
      }),
      ...
      await this.models.call.findAll({
        attributes,
        limit,
        offset,
        order,
    ...
    //    include: [{ model: this.models.audio }]
//include: { model: this.models.audio, as: 'call_audio'}
      ...
      return Object.keys(this.groupsMap).reduce((groups, group) => {
        Object.keys(this.groupsMap[group]).forEach((system) => {
          if (Object.keys(scope).includes(system)) {
            const talkgroups = this.groupsMap[group][system].filter(
                (talkgroup) => scope[system].includes(talkgroup));
  ...
  getGroupsMap(systems = this.config.systems) {
    ...
    systems.forEach((system) => {
      system.talkgroups.forEach((talkgroup) => {
        const group =
            this.config.groups.find((group) => group._id === talkgroup.groupId);
  ...
  getOptions() {
    ...
    const keypadBeeps = this.config.options.keypadBeeps === false ?
        false :
        this.config.options.keypadBeeps === 'uniden' ?
        defaults.keypadBeeps.uniden :
        this.config.options.keypadBeeps === 'whistler' ?
        defaults.keypadBeeps.whistler :
        defaults.keypadBeeps.uniden;
      ...
      return this.config.systems.filter((sys) => sysIds.includes(sys.id))
          .map(
              (sys) => ({
                ...
                order: sys.order,
                talkgroups:
                    sys.talkgroups.filter((tg) => scope[sys.id].includes(tg.id))
                        .map((tg) => ({
                               group: (this.config.groups.find(
                                           (g) => g._id === tg.groupId) ||
                                       {}).label ||
                                   'Unknown',
                               id: tg.id,
    ...
    if (scope !== null && typeof scope === 'object') {
      return Object.keys(this.tagsMap).reduce((tags, tag) => {
        Object.keys(this.tagsMap[tag]).forEach((system) => {
          if (Object.keys(scope).includes(system)) {
            const talkgroups = this.tagsMap[tag][system].filter(
                (talkgroup) => scope[system].includes(talkgroup));
  ...
  async importCall(call = {}, meta) {
    ...
    let talkgroup = Array.isArray(system?.talkgroups) ?
        system.talkgroups.find((tg) => {
          if (tg.id === call.talkgroup) {
            return true;
          } else if ((tg.patches || '')
                         .split(',')
                         .map((id) => +id)
                         .includes(call.talkgroup)) {
    ...
    if (Array.isArray(system?.blacklists) &&
        system.blacklists.includes(call.talkgroup)) {
    ...
    if ((this.config.options.autoPopulate || system?.autoPopulate) &&
        !talkgroup && call.talkgroup) {
      populated = true;
      ...
      if (!group) {
        group = {
          _id: groups.reduce(
                   (id, group) => (group._id > id ? group._id : id), 0) +
              1,
    ...
    if (!this.config.options.disableDuplicateDetection) {
      /*
            ...
            // also more than likely flat out
            // does not work at all for anything

            const dateFrom = new Date(call.dateTime);
            const dateTo = new Date(call.dateTime);
            const delay = this.config.options.duplicateDetectionTimeFrame ??
         defaults.options.duplicateDetectionTimeFrame;
            ...
            if (duplicateCall) {
                this.log.write(Log.warn, [
                    `NewCall: system=${call.system}
         talkgroup=${call.talkgroup}`, `file=${call.audioName} Duplicate call
         rejected`,
                ].join(' '));
      ...
      if (duplicateCall) {
        ...
        if (duplicateCall.rdioScannerAudio == null) {

           const callAudio = await this.models.audio.create({audio: call.audio, call_id: duplicateCall.id });
           this.log.write(Log.warn, `imported call audio for call id ${duplicateCall.id} even though its a duplicate because there was no audio`);
        }
        return;
      }
    }
    let newCall;
    let call_audio_hack = call.audio;
    delete call.audio;
    ...
    this.log.write(
        Log.info,
        `NewCall: system=${call.system} talkgroup=${call.talkgroup} file=${
            call.audioName} Success`);
    this.log.write(Log.info,'imported new call id' + newCall.id);
   
    const callAudio = await this.models.audio.create({audio: call_audio_hack, call_id: newCall.id });
    
    this.log.write(
        Log.info,'imported call audio for call id ' + callAudio.call_id);
    //console.log(newCall);
          // fucking sequelize
          // i hate this shit so much
    newCall.dataValues.audio = call_audio_hack;
  ...
  async importTrunkRecorder(audio, audioName, audioType, system, meta) {
    ...
    await this.importCall({
      ...
      frequencies: Array.isArray(meta.freqList) ?
          meta.freqList.map((f) => ({
  ...
  async messageParser(socket, message) {
    try {
      ...
      console.log('parsing messge...');
      if (message[0] == 'LFM' || message[0] == 'LCL') {
        // dont log it to the console its a waste of space 
        console.log(message[0]);
      } else {
        console.log(message);
        console.log('^^message content above^^');
      }
    } catch (_) {
      message = [];
    }
    ...
    if (Array.isArray(message)) {
      ...
      if (message[0] === wsCommand.ver) {
        socket.send(JSON.stringify([wsCommand.ver, this.version]));
          ...
          this.isAccessRestricted && !socket.access &&
          message[0] !== wsCommand.pin) {
        ...
        socket.send(JSON.stringify([
          wsCommand.listCall,
          await this.getCalls(message[1], socket.scope),
        ]));
        ...
        if (message[1] !== null && typeof message[1] === 'object') {
          const allOff =
              Object.keys(message[1])
                  .every(
                      (sys) => Object.keys(message[1][sys])
                                   .every((tg) => !message[1][sys][tg]));
              ...
              if (socket.readyState !== 3) {
                if (call.system in socket.scope && call.system in message[1]) {
                  if (socket.scope[call.system].includes(call.talkgroup)) {
                    if (message[1][call.system] &&
                        message[1][call.system][call.talkgroup]) {
        ...
        if (socket.authCount > maxAuthenticationTries) {
          ...
          if (socket.access) {
            if (socket.access.expiration instanceof Date &&
                socket.access.expiration.getTime() < new Date().getTime()) {
              this.log.write(
                  Log.warn,
                  `Authentication: ident="${socket.access.ident}" expired`);
            ...
            if (typeof socket.access.limit === 'number') {
              const count =
                  Array.from(this.websocket.wss.clients)
                      .reduce(
                          (c, s) => (s.access?.code === token ? ++c : c), 0);
              ...
              if (count > socket.access.limit) {
                ...
                this.log.write(
                    Log.warn,
                    `Authentication: ident="${socket.access.ident}" ` +
                        `too many concurrent connections, limit is ${
                            socket.access.limit}`);
          ...
          if (socket.access) {
            ...
            socket.send(JSON.stringify(
                [wsCommand.config, this.getConfig(socket.scope)]));
          } else {
            socket.send(JSON.stringify([wsCommand.pin]));
          }
        } else {
    ...
    if (this.config.options.pruneDays > 0) {
      this.pruneInterval = setInterval(() => {
        ...
        const olderThan = new Date(
            ...
            now.getDate() - this.config.options.pruneDays);
  ...
  validateApiKey(apiKey, system, talkgroup) {
    const systems = this.config.options.autoPopulate ||
            this.config.systems.find((sys) => sys.id === system)?.autoPopulate ?
